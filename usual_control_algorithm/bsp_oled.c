...
/**
  ...
  * @param[in]      dat: 要写入的字节
  * @param[in]      cmd: OLED_CMD 代表写入的字节是控制指令; OLED_DATA 代表写入的字节是数据指令
  * @retval         none
  */
void oled_write_byte(uint8_t dat, uint8_t cmd)
{
    ...
    cmd_data[1] = dat;
    // I2C句柄 I2C从机地址 数据指针 数据长度 超时时间
    HAL_I2C_Master_Transmit(&hi2c2, OLED_I2C_ADDRESS, cmd_data, 2, 2);	// 向某个I2C设备传输数据
}
...
/**
  * @brief          操作GRAM内存(128*8char数组) 打开OLED显示
  * @param[in]      pen: 操作类型.
                    PEN_CLEAR: 设置为0x00 代表将所有像素熄灭
                    PEN_WRITE: 设置为0xff 代表将所有像素点亮
                    PEN_INVERSION: 按位取反 代表将所有像素状态反转
  * @retval         none
  */
...
/**
  ...
  * @param[in]      x1: 起点 y1: 起点 x2: 终点 y2: 终点
